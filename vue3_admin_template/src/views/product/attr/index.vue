<template>
  <div>
    ...
    <el-card style="margin: 10px 0px">
      ...
      <div v-show="scene == 1">
        <!-- 像这种“列表页”与“编辑页”来回跳的后台管理界面，v-if将销毁旧的 DOM 节点并重新创建新的组件。
         这涉及大量的生命周期钩子执行、数据重新绑定。如果界面复杂会有卡顿。 -->
        <!-- 展示添加属性与修改数据的结构 -->
        <el-form :inline="true">
...
const ATTR_CATEGORY_STORAGE_KEY = 'product-attr-category-selection'

let categoryStore = useCategoryStore()
//存储已有的属性与属性值
// attrArr 响应式对象的类型是Attr[]类型，初始值为空数组
...
//监听仓库第三级分类ID变化
watch(
  // 监听categoryStore响应式对象的c3Id属性。
  // 这里必须写成函数形式才能实现监听，若写成categoryStore.c3Id，就相当于给watch传入一个死数字
  () => categoryStore.c3Id,
  () => {
    // 这里小括号省略了newValue,oldValue，因为没用到
    //清空上一次查询的属性与属性值
    attrArr.value = []
    //保证三级分类得有才能发请求
...
onMounted(async () => {
  const raw = sessionStorage.getItem(ATTR_CATEGORY_STORAGE_KEY)
  if (!raw) return
  try {
...
// 离开页面前记住三级分类，然后再清空仓库（与 SPU 管理等页面一致）
onBeforeUnmount(() => {
  if (
