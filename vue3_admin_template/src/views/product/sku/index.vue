<template>
  <el-card>
      ...
      <template #default>
        ...
        <el-row style="margin: 10px 0px">
          ...
          <el-col :span="18">
            ...
            <div v-else-if="skuInfo.skuDefaultImg">
              <img
    ...
    <el-drawer v-model="editDrawer" title="编辑 SKU" size="560px">
      <SkuForm ref="skuEditFormRef" @changeScene="onEditSkuFormDone" />
    </el-drawer>
  </el-card>
</template>
...
const skuEditFormRef = ref<{ initSkuEditData: (id: number) => Promise<void> } | null>(
  null,
)
//组件挂载完毕
onMounted(() => {
...
//更新已有的SKU：抽屉内复用 SPU 模块的 SkuForm
const updateSku = async (row: SkuData) => {
  if (row.id == null) {
    ElMessage({ type: 'warning', message: '缺少 SKU 编号，无法编辑' })
    return
  }
  editDrawer.value = true
  await nextTick()
  skuEditFormRef.value?.initSkuEditData(row.id as number)
}

const onEditSkuFormDone = () => {
...
const removeSku = async (id: number) => {
  try {
    ...
    ElMessage({ type: 'error', message: e?.message || '删除失败' })
  }
}
</script>
