...
/** 是否为当前登录用户所在行（与 /info 返回的 name 对齐：可能对应表格里的「用户姓名」或「用户账号」） */
const isCurrentUserRow = (row: User) => {
  const me = (userStore.username || '').trim()
  if (!me) return false
  const account = (row.username || '').trim()
  const displayName = (row.name || '').trim()
  return account === me || displayName === me
}

const rowSelectable = (row: User) => !isCurrentUserRow(row)
//组件挂载完毕
onMounted(() => {
  getHasUser()
})
