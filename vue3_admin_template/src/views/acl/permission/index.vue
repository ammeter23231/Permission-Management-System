...
const removeMenu = async (id: number) => {
  try {
    // 发起请求
    let result = await reqRemoveMenu(id) // 如果请求失败则会跳转到catch继续执行

    // 如果执行到这里，说明 result.code 是 200（拦截器放行了）
    ElMessage({ type: 'success', message: '删除成功' })
    getHasPermisstion()
  } catch (error: any) {
    // 如果后端返回非 200，拦截器会 Promise.reject
    // 错误对象会被 catch 捕获，消息就在 error.message 中
    ElMessage({
      type: 'error',
