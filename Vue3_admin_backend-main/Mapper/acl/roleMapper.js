...
const permissionToAssing = async (roleId) => {
    ...
    try {
        ...
        const menuList = menuRows.map(row => ({
            ...row,
            id: row.menu_id, // 必须有 id 字段，对应 el-tree 的 node-key
            select: false,
            children: []
        }))
...
const DoAssignPermission = async (params) => {
    ...
    try {
        ...
        const menuIdList = permissionId 
            ? String(permissionId).split(',').map(id => Number(id)).filter(id => id > 0) 
            : []
        ...
        if (menuIdList.length > 0) {
            // 使用更安全的批量插入写法
            const values = menuIdList.map(mId => [roleId, mId])
            await connection.query(
