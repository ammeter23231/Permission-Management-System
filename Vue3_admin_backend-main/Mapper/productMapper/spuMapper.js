...
const getSpuSaleAttrList = async (spuId) => {
    ...
    try {
        ...
        const [row] = await connection.query(
            "select spu_sale_attr_id as id, base_sale_attr_id as baseSaleAttrId, sale_attr_name as saleAttrName, spu_id as spuId from spu_sale_attr where spu_id=?", 
            [spuId]
        )
        const result = []
        for (let item of row) {
            // 注意：下面的 SQL 语句中 spu_id=? 后面必须有空格
            const [row1] = await connection.query(
                "select sale_attr_value_id as id, sale_attr_value_name as saleAttrValueName, sale_attr_id as baseSaleAttrId, spu_id as spuId from sale_attr_value where spu_id=? and sale_attr_id=?", 
